...
class MyPitchProjects extends StatefulWidget {
  @override
  _NewboardRoomState createState() => _NewboardRoomState();
}

class _NewboardRoomState extends State<MyPitchProjects>
  ...
  Future<String> getProjects() async {
    try {
      http.Response response = await http.get(USER_PROJECTS+"${currentUser.value.id}");
...
//                      ),
                      ...
                      GestureDetector(
                        onTap: (){
                          proj.Live == "Yes"? null :updateLive(proj);
                        },
                        child: Container(
                          padding: EdgeInsets.all(7),
                          decoration: BoxDecoration(
                              ...
                              borderRadius: BorderRadius.circular(20)),
                          child: Text(
                            proj.Live == "Yes"?"Already Live":"Pitch Now (Go Live)",
                            style: TextStyle(color: Colors.white),
                          ),
                        ),
                      ),
...
//          )
  ...
  void updateLive(AllProj proj) async {
    try {
      http.Response response = await http.get(UPDATE_LIVE+"${proj.id}");

      if (response.statusCode == 200) {

        var resData = jsonDecode(response.body);
        if(resData["message"] == "Updated Successfully"){

          onJoin(proj.pitchCode);
        }
